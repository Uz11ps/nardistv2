...
export class FreeTablesService {
  ...
  async createTable(
    ...
    const host = await this.db.findOne('users', { id: hostId });
    if (!host) {
      throw new Error('Host not found');
    }

    const rating = await this.db.query(
      'SELECT * FROM ratings WHERE "userId" = $1 AND mode = $2',
      [hostId, mode]
    ).then(r => r.rows[0]);

    const hostRating = rating?.rating || 1500;
    ...
    const table: FreeTable = {
      ...
      createdAt: Date.now(),
      hostName: host.nickname || host.firstName || 'Unknown',
      hostRating,
    };
  ...
  async joinTable(tableId: string, playerId: number): Promise<FreeTable | null> {
    ...
    if (table.minRating || table.maxRating) {
      const player = await this.db.findOne('users', { id: playerId });
      if (!player) {
        throw new Error('Player not found');
      }

      const playerRatingResult = await this.db.query(
        'SELECT * FROM ratings WHERE "userId" = $1 AND mode = $2',
        [playerId, table.mode]
      );
      const playerRating = playerRatingResult.rows[0]?.rating || 1500;
