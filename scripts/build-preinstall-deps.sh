#!/bin/bash

# Самый быстрый вариант: предварительно устанавливаем зависимости на сервере,
# затем используем простой Dockerfile который просто копирует готовые файлы

set -e

echo "🚀 Быстрая сборка с предустановленными зависимостями"
echo ""

cd /opt/Nardist

# 1. Обновляем код
echo "📥 Обновляем код из Git..."
git pull origin main

# 2. Устанавливаем зависимости на сервере (быстрее чем в Docker)
echo "📦 Устанавливаем зависимости backend на сервере..."
cd backend
rm -rf node_modules package-lock.json dist

echo "⏳ Это может занять 2-5 минут..."
npm install --legacy-peer-deps --no-audit --progress=false --prefer-offline=false

# 3. Собираем приложение на сервере
echo "🏗️ Собираем приложение на сервере..."
npm run build

# 4. Временно переименовываем .dockerignore (он игнорирует node_modules)
if [ -f .dockerignore ]; then
    mv .dockerignore .dockerignore.backup
fi

# 5. Создаем минималистичный Dockerfile
cat > Dockerfile.fast << 'EOF'
FROM node:18-slim

WORKDIR /app

# Копируем только необходимые файлы
COPY package*.json ./
COPY node_modules ./node_modules
COPY dist ./dist

EXPOSE 3000

HEALTHCHECK --interval=30s --timeout=10s --start-period=40s --retries=3 \
    CMD node -e "require('http').get('http://localhost:3000/api/health', (r) => {process.exit(r.statusCode === 200 ? 0 : 1)})" || exit 1

CMD ["node", "dist/main"]
EOF

# 6. Собираем Docker образ (очень быстро, так как все уже готово)
echo "🐳 Собираем Docker образ (быстро, так как зависимости уже установлены)..."
cd ..
docker build -f backend/Dockerfile.fast -t nardist-backend:latest backend/

# 7. Восстанавливаем .dockerignore
cd backend
if [ -f .dockerignore.backup ]; then
    mv .dockerignore.backup .dockerignore
fi
cd ..

# 8. Останавливаем старые контейнеры (безопасно, игнорируем ошибки если контейнеры уже остановлены)
echo "🛑 Останавливаем старые контейнеры..."
set +e  # Временно отключаем строгий режим для безопасной остановки
docker compose -f docker-compose.prod.yml down --remove-orphans 2>/dev/null || true
docker compose -f docker-compose.prod.yml down --remove-orphans || true
set -e  # Включаем строгий режим обратно

# 9. Запускаем PostgreSQL и Redis сначала
echo "🚀 Запускаем PostgreSQL и Redis..."
docker compose -f docker-compose.prod.yml up -d postgres redis

# 10. Ждем пока PostgreSQL будет готов
echo "⏳ Ждем готовности PostgreSQL..."
sleep 10

# 11. Проверяем и создаем базу данных если нужно
echo "🔍 Проверяем базу данных..."
chmod +x scripts/ensure-database.sh
bash scripts/ensure-database.sh || echo "⚠️  Не удалось проверить базу данных, продолжаем..."

# 12. Запускаем остальные сервисы (используем --no-build чтобы не пересобирать уже собранный образ backend)
echo "🚀 Запускаем остальные сервисы..."
# Убеждаемся что используется локальный образ backend (не из registry)
export BACKEND_IMAGE=nardist-backend:latest
# Запускаем все сервисы с --no-build (backend образ уже собран, остальные используют готовые образы)
docker compose -f docker-compose.prod.yml up -d --no-build

echo ""
echo "✅ Быстрая сборка завершена!"
echo "📝 Логи:"
docker compose -f docker-compose.prod.yml logs --tail=50 backend
